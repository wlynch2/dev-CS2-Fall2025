...
public class TestBillCalculations_tipAmount {
	
	@Test
	void testWhenItemIsEmpty() {
		assertThrows(IllegalArgumentException.class, ()->{BillCalculations.tipAmount(null);});
	}
	...
	void testTipAmountMethod() {
		...
		list[0] = item;
		assertEquals(2.0, BillCalculations.tipAmount(list));
	}
	...
	void testTipWithMultipleItems() {
		...
		list[0] = item;
		list[1] = item2;
		assertEquals(4.0, BillCalculations.tipAmount(list));
	}
